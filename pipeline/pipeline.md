# Live pipeline

**Single source of truth. If it isn't here, it doesn't exist — and things that didn't exist
are exactly what has been costing the money.**

Update on every movement. Never delete a row; move it to Closed with an outcome.

---

## Stages

| Stage | Meaning | Exit condition |
|---|---|---|
| `INBOX` | Inbound arrived, not yet answered | Answered — **within 24h, always** |
| `QUOTED` | Rate given, waiting on brand | Brand responds |
| `NEGOTIATING` | Brand countered | Agreed, or escalated to Sid |
| `WON` | Terms agreed, date set | Placement goes live |
| `DELIVERING` | Sid producing / scheduled | Live |
| `INVOICED` | Live, payment requested | Paid |
| `PAID` | Cash in | — |
| `LOST` | Dead | Log the reason |

⚠️ **`INBOX` is the dangerous stage.** Everything lost in 2026 died here. Nothing sits >24h.

---

## Active

| ID | Brand | Rail | Contact | Stage | Placement | Value | Next action | Due | Updated |
|---|---|---|---|---|---|---:|---|---|---|
| ~~D1~~ | ~~**Luma**~~ | 1stCollab | Tarak Duggal — `tarak@1stcollab.com` | **`LOST`** | 1x LinkedIn video post | **$2,000** | **Moved to LOST (L5).** Draft B (pass + pitch Q4) still worth sending to keep the rail warm | — | 4 Sep 2026 |
| D2 | **Paved bundles** | Paved | Yuvna Muthy Nunes | `QUOTED` | Q4 newsletter + LinkedIn bundles, listing | up to $5,500 | **→ SID, highest-value action on the desk.** Carries 5 re-pitches, and a listed bundle is the ONLY lane that earns during the 7–28 Sep absence | **overdue — 17 days silent** | 7 Sep 2026 |
| D3 | **Vercel** | Direct | Nicolas Kaden, Partnerships EMEA | `QUOTED` | Q4 LinkedIn / newsletter | $1,500 | Sent 26 Aug, no reply. T2 overdue; will lapse during the absence | overdue | 7 Sep 2026 |
| D4 | **Enterpret** | Direct | `marketing@` + Sid's WhatsApp thread | `QUOTED` | Q4 LinkedIn / newsletter | $1,500 | T2 chase drafted → SID. Skip if WhatsApp is live | overdue | 3 Sep 2026 |
| D6 | **Atlassian** | Direct | Christopher Metoyer — `cmetoyer2@atlassian.com` **verified 8 Sep** | `INBOX` | LinkedIn post before the 22 Sep summit | $1,500 | **Two drafts → SID.** A (summit) best before ~15 Sep; **B (7 placements in 3 weeks, no summit) works after** — the prospect survives the deadline | ~15 Sep for A | 10 Sep 2026 |
| **D8** | **Outskill** | Direct | **Sowmya C, Partnerships Manager** — `sowmya.c@outskill.com` | **`NEGOTIATING` — live** | Newsletter main ad placement, 3-hour AI tools workshop | **$750+** (multi-edition possible) | **→ SID: send the demographics reply, drafted 22 Sep.** She has now asked **five times** | **OVERDUE — 22 days** | 22 Sep 2026 |
| **D9** | **Tiiny Host** | Direct | Marjorie Palanee (+ Elston) | **`DELIVERING` — time-boxed** | Ambassador programme post | ? ⚠️ terms unconfirmed | **→ SID: timeline reply drafted 24 Sep** proposing draft 30 Sep, live 2 Oct. They wrap phase 1 next week | **next week** | 24 Sep 2026 |
| D11 | **Mark Progano** | Newsletter reply | `mark@mprogano.com` | `INBOX` | Unspecified — "would you want to collab?" | ? | Qualifying reply drafted 24 Sep, states card rates | — | 24 Sep 2026 |
| D10 | **Wooly Network** | InboxReads | I. Petronel, Co-founder | `INBOX` | Affiliate marketplace | ? | Unread reminder 21 Sep. Triage on return | — | 22 Sep 2026 |
| D7 | **Pendo** | Direct | Jennifer Peterson, Sr Dir Marketing, Global Brand & Content — `jennifer.peterson@pendo.io` **verified** | `INBOX` | Q4 LinkedIn / newsletter | $1,500 | **Email drafted 11 Sep** → SID. Best-qualified prospect on the desk | — | 11 Sep 2026 |
| D5 | **Product-Led Alliance** | Direct | Fiona Standen — `fiona@pmmalliance.com` | `INBOX` (dormant since Mar) | Q4 LinkedIn / newsletter | $1,500 | **Re-pitch drafted 3 Sep** → SID. ⚠️ If she counters with a promo swap, that's Sid's call | — | 3 Sep 2026 |

---

## Won / delivering

| ID | Brand | Rail | Placement | Value | Live date | Invoiced | Paid |
|---|---|---|---|---:|---|---|---|
| — | | | | | | | |

---

## Closed — lost

| ID | Brand | Rail | Value | Reason | Date |
|---|---|---|---:|---|---|
| L1 | Promoted | Passionfroot | $1,200 | Expired unanswered — **recoverable**, see B5 | May 2026 |
| L2 | Coworker.ai | Passionfroot | $480 | Expired unanswered — **recoverable**, see B6 | May 2026 |
| L3 | Rezonant | Passionfroot | ? | Never answered — **recoverable**, see B7 | Apr 2026 |
| L4 | Recall | 1stCollab | ~$1,200 | Match never actioned — **recoverable**, see B8 | Apr 2026 |
| **L5** | **Luma** (Layers) | **1stCollab** | **$2,000** | **Rush campaign, 7 Sep live date. Arrived 28 Aug, reply drafted 31 Aug, never sent. Expired unanswered.** | **Sep 2026** |

> **All five died the same way: nobody replied.** Combined, ~$5–6K. This table is here as a
> standing argument for the 24-hour rule.
>
> **L5 is the one that matters, because the desk was running.** Promoted, Coworker.ai, Rezonant
> and Recall all died before there was a desk. Luma died *with* a finished draft sitting in
> Gmail for four days and a daily report naming it as the top priority. The bottleneck is not
> sourcing, not contact discovery, not drafting, and not knowing what to do. **It is the send.**

---

## Month tracker

**Target: $8,000–10,000/month.**

| Month | Booked | Delivered | Collected | vs target |
|---|---:|---:|---:|---|
| **Aug 2026** | **$0** | $0 | $0 | **CLOSED — missed $2,000 pace target** |
| Sep 2026 | $0 | $0 | $0 | **$10,000 cumulative pace target** |

**August closed at $0.** The first cold email went out 19 Aug and nothing cold closes inside
two weeks, so this was the expected miss — but it is still a miss, and September has to carry
what August didn't.

**D1 (Luma, $2,000) is the first genuinely September-bookable deal on the desk.** It is also
the one with an expiry date measured in days.

Booked = terms agreed. Delivered = placement live. Collected = cash received. Track all three
— the 2026 problem has been gaps between them, not just a shortage of bookings.

---

## Conventions

- **IDs:** `D1, D2…` deals · `B1, B2…` backlog · `L1, L2…` lost · `O1, O2…` outbound prospects
- **Value:** quoted amount, gross. Note marketplace fees separately — gross ≠ net.
- **Due:** every active row needs a next action *and* a date. A row without both is a row
  about to become an `L`.
- **Escalations:** mark `→ SID` in Next action for anything needing his decision (any
  discount, custom bundle, date commitment, or non-cash offer).
